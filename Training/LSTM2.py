...
model = Sequential()
model.add(keras.layers.Embedding(len(word2id) + 1,
                                        embedding_dim,
                                        input_length=max_words)

model.add(embedded_inputs = keras.layers.Dropout(0.2))
model.add(keras.layers.wrappers.Bidirectional(
    keras.layers.LSTM(embedding_dim, return_sequences=True)
))
model.add(keras.layers.Dropout(0.2))
model.add(keras.layers.Permute((2, 1)))
model.add(keras.layers.TimeDistributed(keras.layers.Dense(1)))
model.add(keras.layers.Reshape((max_words,)))
model.add(keras.layers.Activation('softmax', name='attention_vec')(attention_vector))
model.add(keras.layers.Dot(axes=1))
model.add(keras.layers.Dense(embedding_dim, activation='relu'))
model.add(keras.layers.Dense(len(label2id), activation='softmax'))
...
model.save("./msg_flag_epochs=2, batch_size=128, validation_split=0.1,")
print("Saved!")
